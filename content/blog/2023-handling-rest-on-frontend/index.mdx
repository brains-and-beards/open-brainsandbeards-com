---
path: '/blog/2023-handling-rest-on-frontend'
date: '2023-05-17T19:12:41.253Z'
title: 'Handling REST on Frontend'
image: header.jpg
imageCaption: 'Network cables photo by <a href="https://unsplash.com/photos/40XgDxBfYXM">Jordan Harrison</a>'
...
So now, the only thing that's left is to ask the server for our `joke`. From now on, there are number of valid options for calling the API from the component. In real world applications, there is always some state management library ([redux](https://redux.js.org) / [mobx](https://mobx-state-tree.js.org/intro/welcome)) in use, and we usually offload async logic like that to their corresponding middlewares.

That said the simples way for rendering our remote data inside the component is to just call the functions I mentioned above, in some button `onPress` handler or `useEffect()` hook, save the results in local state and voilà. Just be sure to always wrap your API calls with `try-catch` block

```tsx
function useJokeRequest() {
  const [joke, setJoke] = useState<JokeBE>()
...
We can also go a step further and combine our request related logic into a single custom hook like above. This way we get the joke on mount, loading state to display a loader while our request is in fly, and error in case the request failed.

Depending on the needs, if we find repetetive paterns e.g. we're doing on mount API calls and we always use error / loading states, nothing stops us from extracting common logic to some more universal reusable custom hook.  
Once we got our first API call down, sky is the limit!

You can find the full code example used in this article under [this link](https://github.com/brains-and-beards/rn-rest-example)

*btw. I got quiet surprised when I got this from the API 😂:*
